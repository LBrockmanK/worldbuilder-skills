...
# RPG World Foundation

## Overview

A world for an LLM-powered game is not a backdrop — it is a behavioral context. Every character draws meaning from their relationship to the setting's wound, the community's personality, and each other's households. Design structure first; characters emerge from it.

Work through the six foundational questions before touching any character card or lorebook entry. They prevent the most common failure modes: a cast that has no stakes, a community that has no personality, and a player whose presence means nothing.
...
If the setting includes supernatural elements, the origin shapes tone significantly:
...
If the setting has no magic, this becomes: what is the setting's concealed depth? Every good setting has something beneath the surface, even in mundane worlds — a secret, a history, a truth the surface doesn't advertise.

### 5. What is the era?

Approximate technology and cultural reference point. This affects every character's daily life and what kinds of problems are plausible:
...
## Household Design Template

For each household before assigning characters:
...
Inter-household connections: which other households are they tied to, and how
Narrative hook: what is interesting about this household as a unit
Trajectory: where are they headed if nothing changes — what is the slow drift of their situation
```
...
- A trajectory — this is what makes household dynamics feel like they exist in time rather than static tableau
...
**Anti-redundancy check:** If two characters occupy the same slot, verify they differ substantially in execution — different emotional register, different obstacle, different function in the setting. The same archetype expressed the same way produces redundancy that players notice. Two "initially hostile" characters are fine if one's coldness comes from grief and the other's from professional rivalry — they're not actually doing the same thing.
...
**All characters get equal depth.** The main/side toggle is a player-facing engagement setting, not a signal to write a thinner character card. Side characters who feel shallow make the world feel thin.
