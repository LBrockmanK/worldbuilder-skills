...
Minor closed after the branch review: the `SKILL.md` Relationships coverage bullet was the heaviest item in its list at roughly 500 characters. Trimmed to 384 by cutting the sentence that restated the *One question at a time* technique rule four lines above it, which was the same restatement defect this plan removed elsewhere. The separate-questions signal survives as three words, and the absence rule as one clause. It remains the longest bullet in the list, which its content justifies - Intimate Dynamics is 226.
